...
import { FaSave, FaRegStar, FaStar } from 'react-icons/fa'
    ...
    useEffect(() => {
        axios('/waste-containers/' + id, {
            ...
            if (res.status === 200) {
                ...
                let favoriteData = localStorage.getItem(`${username}favorite`) || ''
                if(favoriteData.split(' ').includes(`${id}`)) setIsFavorite(true)
                setLoading(false)
                        ...
                        <div style={{margin:'10px', marginLeft:'0px'}}>
                            <div style={{display: 'flex', flexDirection: 'row', 
                                ...
                                <div onClick={() => {
                                    ...
                                    console.log(`${id}`)
                                    if(!isFavorite){ 
                                        favoriteData = favoriteData.concat(` ${id}`)
                                    }
                                    else{ favoriteData = favoriteData.replace(` ${id}`,'')}
                                    console.log('favoriteDAta:',favoriteData)
                                    localStorage.setItem(`${username}favorite`,favoriteData)
                                }}
